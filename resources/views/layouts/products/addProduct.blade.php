...
<div class="container_content">
    <h3 class="container_content-products">Add Product</h3>

    <form class="form_create_profile" method="POST" action="{{route('admin.addProduct')}}"  enctype="multipart/form-data">
   
        <table class="form_create_table">
            <tr>
                <td class="forn_create_head">Name *</td>
                <td><input required type="text" name="fullname" placeholder="Tên sản phẩm: " id="fullname" value="{{empty($tenSanPham)?'':$tenSanPham}}"> 
                </td>
            </tr>         
            <tr>
                <td class="forn_create_head">PriceSale *</td>
                <td><input required type="number" name="PriceSale" min="1000" id="PriceSale"  value="{{empty($priceSale)?'000':$priceSale}}"
                        placeholder="Nhập giá bán:"></td>
            </tr>
            <tr>
                <td class="forn_create_head">PriceOrigin *</td>
                <td><input required type="number" name="PriceOrigin" min="0" id="PriceOrigin"  value="{{empty($priceOrigin)?'000':$priceOrigin}}"
                        placeholder="Nhập giá gốc:"></td>
            </tr>
            <tr>
                <td class="forn_create_head">Amount *</td>
                <td><input required type="text" name="amount" min="1" id="amount" value="{{empty($soLuong)?'100':$soLuong}}" 
            ...
            <tr>
                ...
                <td>
                    <select style="width: 100%; font-size:16px; padding:4px 8px;" value="{{empty($maLoaiHang)?'':$maLoaiHang}}" name="maLoaiHang" id="">
        ...
        <table class="form_create_table">
            ...
            <tr>
                ...
                <td><textarea name="moTa" placeholder="Mô tả sản phẩm:" value="{{empty($moTa)?'':$moTa}}" id="" style="width: 100%; color:wheat;" rows="10"></textarea></td>
            </tr>
            <tr >
...
@section('directory')
> <a href="{{route('admin.showAllProduct')}}">Products</a> > <a href="#">Add Products</a>
@endsection
@section('message') 
    @if(!empty($message))
    <h2 >{{$message}}</h2>
